...
if ($productId <= 0 || $name === '' || $category === '' || $price === null || $quantity === null) {
...
$check = $conn->prepare('SELECT id, image_url FROM products WHERE id = ? AND is_deleted = 0' . ($isAdmin ? '' : ' AND farmer_id = ?'));
if ($isAdmin) {
...
if ($uploadPath === '') {
    $uploadPath = $existing ? (string) $existing['image_url'] : '';
}

$farmerId = null;
if ($isAdmin && isset($input['farmer_id'])) {
...
if ($farmerId !== null) {
    $stmt = $conn->prepare('UPDATE products SET farmer_id = ?, name = ?, category = ?, price_kes = ?, quantity = ?, quantity_label = ?, description = ?, image_url = ?, status = ? WHERE id = ?');
    $stmt->bind_param('issdissssi', $farmerId, $name, $category, $price, $quantity, $quantityLabel, $description, $uploadPath, $status, $productId);
} else {
    $stmt = $conn->prepare('UPDATE products SET name = ?, category = ?, price_kes = ?, quantity = ?, quantity_label = ?, description = ?, image_url = ?, status = ? WHERE id = ?');
    $stmt->bind_param('ssdissssi', $name, $category, $price, $quantity, $quantityLabel, $description, $uploadPath, $status, $productId);
}

if ($stmt->execute()) {
    audit_log($conn, 'update', 'product', $productId, 'Product updated');
    json_response(['success' => true, 'message' => 'Product updated']);
}

json_response(['success' => false, 'message' => 'Product update failed'], 500);
