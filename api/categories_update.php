...
if ($id <= 0 || $name === '') {
    json_response(['success' => false, 'message' => 'Category ID and name required'], 400);
}
...
if ($stmt->execute()) {
    audit_log($conn, 'update', 'category', $id, 'Category updated');
    json_response(['success' => true, 'message' => 'Category updated']);
}

json_response(['success' => false, 'message' => 'Category update failed'], 500);
